...
INSERT INTO %[1]s (v) values ($1);`

// Insert adds a new row to the table.
func (t *KvTable) Insert(ctx *stopper.Context, conn *pgxpool.Conn, value string) error {
	_, err := conn.Exec(ctx, fmt.Sprintf(insertTableStmt, t.String()), value)
	return err
}

const restoreTableStmt = `
RESTORE %[1]s  FROM '%[2]s' IN 'external://%[3]s' WITH into_db=%[4]s 
`
...
func (t *KvTable) Fingerprint(ctx *stopper.Context, conn *pgxpool.Conn) (string, error) {
	...
	query := fmt.Sprintf("SHOW EXPERIMENTAL_FINGERPRINTS FROM TABLE %s", t.String())
