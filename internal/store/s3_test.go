...
func TestS3FromEnv(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			env := &env.Env{Path: "bucket/key", Endpoint: tt.endpoint, LookupEnv: lookup}

			store, err := S3FromEnv(env)
			if tt.wantErr != nil {
				assert.Nil(t, store)
			...
			assert.Regexp(t, "^bucket/key", s3.dest)
		})
	}
}
